...
export default function AccountDetails() {
  ...
  // avg income
  const incomeTransactions = account?.data?.transactions?.filter(
    (transaction) => transaction?.transactionType === "Income"
  );
  const totalIncomee = incomeTransactions.reduce((acc, transaction) => {
    return acc + transaction?.amount;
  }, 0);
  const averageIncome = totalIncomee / incomeTransactions.length;
